...
const trainedSentiment= new TextAnalyzer();
fs.createReadStream('SentimentTrain.csv')
.pipe(csv())
.on('data',function(data){
    //console.log("Quote is:"  + data.text);
    //console.log("Label is: " + data.label);
    trainedSentiment.train([{
        text: data.text,
        label: data.label
    }]);
})
.on('end', () => {
    console.log("Vocab size:", trainedSentiment.vocab.size);
    console.log("Total quotes:", trainedSentiment.total_quotes);    
...
module.exports = trainedSentiment